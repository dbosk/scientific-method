...
\citetitle{HEO2}~\cite{HEO2}, namely:
\begin{frame}[fragile]
...
\begin{frame}[fragile]
  ...
  \only<presentation>{\begin{itemize}}
    \item\label{LOrelate} relate the different parts of scientific method, how 
      they relate to one another, contribute and not contribute to 
      scientificity in security
      \only<article>{[\ref{K1}, \ref{K2}, \ref{J2}]}
    \item\label{LOevaluate} assess, analyse, and discuss the quality in, and 
      ethical aspects of, knowledge generation related to digital systems and 
      in particular the security of these systems
      \only<article>{[\ref{C1}, \ref{C2}, \ref{J1}, \ref{K3}]}
    \item\label{LOapply} apply scientific methodology to show how to answer 
      issues in the cybersecurity field
      \only<article>{[\ref{C1}, \ref{C2}]}
    \only<presentation>{%
    \item \dots
    }
    \only<article>{%
    \item\label{LOplan} plan and carry out assignments within given time frames 
      and available resources [\ref{C1}]
    \item\label{LOcomm} write short, clear and arguing texts based on own 
